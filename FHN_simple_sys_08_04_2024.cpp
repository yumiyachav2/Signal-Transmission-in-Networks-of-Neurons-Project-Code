...
// globals
double AMP = 5, ALPHA = 1, BETA = 1, GAMMA = 1, STEP = 5e-5, TIME = 1e-2, MANUAL_DIFF_STEP = 1e-4;
...
int main()
{
    ...
    for (int i = 0; i <1000; i++) //i<n, n=number of points in dataset
    {
        double val1 = dist_u(rnd), val2 = dist_u(rnd);
        MVector x = {val1, val2}, y = {static_cast<double>(static_cast<int>(val1*val1 + val2*val2 < 1))};
        X_train.push_back(x);
    ...
    std::vector<int> l_size = {30}; //include the different numbers of neurons per layer here
    ...
    int BATCH_SIZE = 10, ITERS = 101, count;
    ...
    std::string base = "C:\\Users\\Andrzej Socha\\Desktop\\FHN NN Results\\Base\\expanded model round 3, turn 3, eps ge 1e-2, 5000 iters lr 2e-3 circular ", file_name; //path to save the cost and accuracy stats
    file_name = std::to_string(l_size[ls]);
    ...
    std::string boundary = "C:\\Users\\Andrzej Socha\\Desktop\\FHN NN Results\\Base\\expanded model, 30 n, 1000 test, equivalent of 1000 iters, boundary round.txt"; //path to save the decision boundary stats 
    MVector dec;
    std::ofstream fb(boundary);
    for (int x = 0; x <= 50; x++)
    {
        for (int y = 0; y <= 50; y++)
        {
